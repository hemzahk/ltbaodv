...
Define_Module(TrustAodv);


const int KIND_DELAYEDSEND = 100;

void TrustAodv::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        EV_INFO <<"[TrustAodv] Initialized."<< endl;
    }
...
void TrustAodv::handleMessageWhenUp(cMessage *msg)
{
...
void TrustAodv::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void TrustAodv::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result TrustAodv::ensureRouteForDatagram(Packet *datagram)
{
...
bool TrustAodv::hasOngoingRouteDiscovery(const L3Address& target)
{
    return containsKey(waitForRREPTimers, target);
}

void TrustAodv::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address TrustAodv::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void TrustAodv::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void TrustAodv::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void TrustAodv::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> TrustAodv::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> TrustAodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> TrustAodv::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void TrustAodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    ...
    double previousHopTrust   = trustManager->getTrustScore(sourceAddr);

    if(previousHopTrust < 0.5)
        return;

    //double incomingRouteTrust = rrep->getRouteTrust() * previousHopTrust;

    double incomingRouteTrust = std::min(rrep->getRouteTrust() , previousHopTrust);
    ...
    if (!previousHopRoute || previousHopRoute->getSource() != this) {
        ...
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(previousHopRoute->getProtocolData());
        routeData->setRouteTrust(previousHopTrust);
    ...
    if (destRoute && destRoute->getSource() == this) { // already exists
        ...
        if (!destRouteData->hasValidDestNum()) {
                ...
                destRouteData->setRouteTrust(incomingRouteTrust);
        ...
        //      known value is valid, or
            ...
            //      smaller than the hop count in route table entry.
                      ...
                      (destSeqNum == destRouteData->getDestSeqNum() && destRouteData->getRouteTrust() < incomingRouteTrust) ||
                      (destSeqNum == destRouteData->getDestSeqNum() && destRouteData->getRouteTrust() == incomingRouteTrust && newHopCount < (unsigned int)destRoute->getMetric())) {
                ...
                if(destRouteData->getRouteTrust() < incomingRouteTrust ||
                    (destRouteData->getRouteTrust() == incomingRouteTrust && newHopCount < (unsigned int)destRoute->getMetric())) {
                        updateRoutingTable(destRoute, sourceAddr, newHopCount, true, destSeqNum, true, simTime() + lifeTime);
                        destRouteData->setRouteTrust(incomingRouteTrust);
                }
        ...
        if (originatorRoute && originatorRoute->getSource() == this) {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0) {
                ...
                auto outgoingRREP = dynamicPtrCast<Rrep>(rrep->dupShared());
                    ...
                    outgoingRREP->setRouteTrust(incomingRouteTrust);
...
void TrustAodv::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void TrustAodv::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void TrustAodv::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void TrustAodv::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void TrustAodv::socketClosed(UdpSocket *socket)
{
    if (operationalState == State::STOPPING_OPERATION)
        startActiveOperationExtraTimeOrFinish(par("stopOperationExtraTime"));
}

void TrustAodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    double newRouteTrust = rreq->getRouteTrust() * trustManager->getTrustScore(sourceAddr);
    EV_INFO << "AODV Route Request arrived with source addr: " << sourceAddr << " originator addr: " << rreq->getOriginatorAddr()
            << " destination addr: " << rreq->getDestAddr()
            << " route trust: " <<rreq->getRouteTrust()
            << " trust value: " << trustManager->getTrustScore(sourceAddr) << endl;
    ...
    // [TrustAodv] When a node receives a RREQ it extracts the route trust value
    // carried in the RREQ.
    ...
    //double incomingRouteTrust = rreq->getRouteTrust() * previousHopTrust;

    double incomingRouteTrust = std::min(rreq->getRouteTrust() , previousHopTrust);
    ...
    if (checkRREQArrivalTime != rreqsArrivalTime.end() && simTime() - checkRREQArrivalTime->second <= pathDiscoveryTime) {
        ...
        if(reverseRoute && reverseRoute->getSource() == this) {
            ...
            if(rreqSeqNum == routeSeqNum) {
                ...
                double existingRouteTrust = routeData->getRouteTrust();
                if(existingRouteTrust < incomingRouteTrust ||
                    (existingRouteTrust == incomingRouteTrust && hopCount < routeHopCount)) {
                        EV_INFO <<"[TrustAodv] Selecting better route via= " << sourceAddr
                                <<" rreqSeqNum= " << rreqSeqNum
                                <<" hopCount= " << hopCount
                                <<" incomingRouteTrust= " << incomingRouteTrust
                                <<" OVER via= " << reverseRoute->getNextHopAsGeneric()
                                <<" routeSeqNum= " << routeSeqNum
                                <<" hopCount= " << routeHopCount
                                <<" existingRouteTrust= " << existingRouteTrust << endl;
                        updateRoutingTable(reverseRoute, sourceAddr, hopCount, true, newSeqNum, true, newLifeTime);
                        routeData->setRouteTrust(incomingRouteTrust);
                }
            }
        }
    ...
    if (!reverseRoute || reverseRoute->getSource() != this) { // create
        ...
        int routeHopCount = reverseRoute->getMetric();
        // double routeTrust = routeData->getRouteTrust();
    ...
    if (timeToLive > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0)) {
        ...
        outgoingRREQ->setRouteTrust(incomingRouteTrust);
...
IRoute *TrustAodv::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void TrustAodv::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void TrustAodv::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> TrustAodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void TrustAodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void TrustAodv::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void TrustAodv::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void TrustAodv::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void TrustAodv::clearState()
{
...
void TrustAodv::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void TrustAodv::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void TrustAodv::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void TrustAodv::completeRouteDiscovery(const L3Address& target)
{
...
void TrustAodv::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> TrustAodv::createHelloMessage()
{
...
void TrustAodv::sendHelloMessagesIfNeeded()
{
...
void TrustAodv::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void TrustAodv::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result TrustAodv::datagramForwardHook(Packet *datagram)
{
...
void TrustAodv::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void TrustAodv::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool TrustAodv::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> TrustAodv::createRREPACK()
{
...
void TrustAodv::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void TrustAodv::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void TrustAodv::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void TrustAodv::handleBlackListTimer()
{
